...
type
    TFormato = (erCNPJ, erCPF, erInscricaoEstadual, erCNPJouCPF, erTelefoneFixo, erCelular, erFixoMovel, erPersonalizado,
...
begin
    ...
    begin
        ...
        // Fixo e Celular...
        if formato = erFixoMovel then
            if Length(SomenteNumero(Texto)) <= 10 then
                texto := Mask('(##) ####-####', SomenteNumero(Texto)) else
                texto := Mask('(##) #####-####', SomenteNumero(Texto));
